    ...
    public:
        ...
        static void ApplyConfiguration( GenApi::INodeMap& nodemap)
        {
            ...
            // frames.
            {
                ...
                // Turn trigger mode on for all trigger selector entries except for the frame trigger given by triggerName.
                for ( GenApi::NodeList_t::iterator it = triggerSelectorEntries.begin(); it != triggerSelectorEntries.end(); ++it)
                {
                    // Set trigger mode to on if the trigger is available.
                    ...
                    if ( IsAvailable( pEntry))
                    {
                        ...
                        if ( triggerName == triggerNameOfEntry)
                        {
                            ...
                            //// Set camera exposure mode to the width of camera trigger signal
                            ...
                            ///Turn test image off
                            testImageSelector->FromString("Off");

                            ///Change the pixel format to Mono12
